...
class Solution:
    ...
    def build_graph(self, words):
        ...
        # initialize graph -> create word set individually
        for word in words:
            for c in word:
                if c not in graph:
                    graph[c] = set()   # 'w': set(), 'r': set(),...
...
if __name__ == '__main__':
    sol = Solution()
    result = sol.alienOrder(["wrt","wrf","er","ett","rftt"])
    print(result)
    result = sol.alienOrder(["z","x"])
    print(result)
    result = sol.alienOrder(["z","x","z"])
    print(result)